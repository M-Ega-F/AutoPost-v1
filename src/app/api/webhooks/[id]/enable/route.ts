...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  try { return apiSuccess({ webhook: await setWebhookActive(userId, id, true) }); } catch (error) { return apiErrorFromUnknown(error, "We couldn't enable this webhook. Try again."); }
}
